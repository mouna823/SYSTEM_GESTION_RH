...
public class EmployeeDashboardController {
    ...
    private void setupBulletinsTable() {
        ...
        bulletinActionsColumn.setCellFactory(param -> new TableCell<>() {
            private final Button btn = new Button("Voir Reçu");
            ...
            protected void updateItem(Void item, boolean empty) {
                ...
                if (empty) {
                    ...
                    btn.setStyle("-fx-background-color: #059669; -fx-text-fill: white; -fx-cursor: hand;");
                    btn.setOnAction(event -> {
                        // Récupérer l'objet bulletin de la ligne
                        BulletinPaie b = getTableView().getItems().get(getIndex());
                        System.out.println("Clic détecté pour le bulletin : " + b.getPeriode()); // Pour déboguer

                        // Création de l'alerte
                        Alert alert = new Alert(Alert.AlertType.INFORMATION);
                        alert.setTitle("Détails du Bulletin");
                        alert.setHeaderText("Bulletin de paie - " + b.getPeriode());

                        // Contenu avec ton genererRecu()
                        TextArea textArea = new TextArea(b.genererRecu());
